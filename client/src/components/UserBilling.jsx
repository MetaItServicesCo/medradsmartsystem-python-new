...
const UserBilling = () => {
  ...
  return (
    <div className="bg-gray-50 min-h-screen w-full max-w-full overflow-x-hidden p-4 md:p-10 font-sans text-gray-700 box-border">
      ...
      {/* Stats Section */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10 bg-white p-6 rounded-xl shadow-sm border border-gray-100 w-full min-w-0">
        {stats.map((stat, index) => (
          <div
            key={index}
            className={`border-l-4 ${stat.color} pl-5 py-2 min-w-0`}
          >
            <p className="text-[10px] tracking-widest text-gray-400 font-bold mb-1">
              {stat.label}
            </p>
            <p className="text-2xl font-black text-slate-800 break-all">
              {stat.value}
            </p>
          </div>
        ))}
      ...
      {/* Monthly Breakdown */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-20 gap-y-4 mb-12 px-2 w-full min-w-0">
      ...
      {/* Activity Section */}
      <div className="rounded-xl overflow-hidden border border-gray-200 shadow-sm bg-white w-full min-w-0">
        {/* Mobile Card View */}
          ...
          {activities.map((item, index) => (
            <div key={index} className="p-5 space-y-3">
              <div className="flex items-center gap-4 min-w-0">
                ...
                <div className="shrink-0 text-[10px] font-bold text-slate-900 text-right">
                  {item.time}
                </div>
              </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {activities.map((item, index) => (
                ...
                >
                  ...
                  <td className="p-5 text-xs font-black text-slate-900 text-right whitespace-nowrap">
                    {item.time}
                  </td>
                </tr>
              ))}
